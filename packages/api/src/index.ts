...
interface MarketplaceAgent {
  ...
  tags: string[];
  tier: string;
  pricing: { model: "free" | "per-task" | "subscription"; amount?: number; currency?: string };
  reputation: { score: number; tasks: number; successRate: number };
  endpoints?: { type: string; url: string }[];
  registeredAt: number;
  lastSeen: number;
  reviews: { reviewer: string; rating: number; comment: string; timestamp: number }[];
  hires: number;
}

const marketplaceAgents = new Map<string, MarketplaceAgent>();

// Seed marketplace with built-in agents
(function seedMarketplace() {
  const seeds: MarketplaceAgent[] = [
    {
      did: "did:sol:5MYxJjeKUq5Di7gqt8Sta7LnJVJMCHPqbxBLiLjNqHQP",
      name: "Kage Core",
      description: "The primary Kage agent — encrypted memory vault, ZK proofs, shielded payments, and multi-agent coordination on Solana.",
      capabilities: ["memory", "reasoning", "messaging", "shielded-payment", "zk-proofs", "delegation"],
      ...
      lastSeen: Date.now(),
      reviews: [
        { reviewer: "did:sol:Demo1", rating: 5, comment: "Rock-solid encryption. Never lost a memory.", timestamp: Date.now() - 86400000 * 7 },
        { reviewer: "did:sol:Demo2", rating: 5, comment: "ZK proofs work flawlessly on devnet.", timestamp: Date.now() - 86400000 * 3 },
      ],
      hires: 184,
    },
    {
      ...
      name: "Shadow Analyst",
      description: "Research agent specialized in on-chain data analysis. Stores findings in encrypted vault and produces verifiable research reports.",
      ...
      reviews: [
        { reviewer: "did:sol:Demo3", rating: 4, comment: "Great analysis, detailed reports.", timestamp: Date.now() - 86400000 * 5 },
      ],
      hires: 89,
    },
    {
      ...
      name: "Phantom Guard",
      description: "Security-focused agent that audits smart contracts, monitors wallet activity, and flags suspicious transactions — all with encrypted logging.",
      ...
      reviews: [
        { reviewer: "did:sol:Demo4", rating: 5, comment: "Found a critical vulnerability others missed.", timestamp: Date.now() - 86400000 * 2 },
      ],
      hires: 52,
    },
    {
      ...
      description: "Multi-agent coordinator that manages task delegation across agent swarms. Tracks reputation and ensures encrypted handoffs.",
    ...
    },
    {
      ...
      name: "Shadow Wallet",
      description: "DeFi privacy agent — executes shielded SOL transfers, scans stealth payments, and manages private portfolio tracking.",
      ...
      reviews: [
        { reviewer: "did:sol:Demo5", rating: 4, comment: "Stealth payments work great. UI could be better.", timestamp: Date.now() - 86400000 },
      ],
      hires: 34,
    },
    {
      ...
      name: "Kage Notary",
      description: "Credential issuance agent — verifies agent capabilities and issues on-chain verifiable credentials for the Kage ecosystem.",
      ...
      lastSeen: Date.now(),
      reviews: [
        { reviewer: "did:sol:Demo6", rating: 5, comment: "Instant credential issuance. Very reliable.", timestamp: Date.now() - 86400000 * 4 },
        { reviewer: "did:sol:Demo7", rating: 5, comment: "Essential service for the ecosystem.", timestamp: Date.now() - 86400000 },
      ],
      hires: 378,
    },
  ];
...
app.get("/marketplace/agents", (_req, res) => {
  const { tag, capability, sort, q } = _req.query as Record<string, string | undefined>;
  ...
  if (sort === "reputation") agents.sort((a, b) => b.reputation.score - a.reputation.score);
  else if (sort === "hires") agents.sort((a, b) => b.hires - a.hires);
  else if (sort === "newest") agents.sort((a, b) => b.registeredAt - a.registeredAt);
...
app.post("/marketplace/register", (req, res) => {
  const { did, name, description, capabilities, tags, pricing, endpoints } = req.body;
  if (!did || !name || !description) {
    return res.status(400).json({ error: "did, name, and description are required" });
  }
...
app.post("/marketplace/agents/:did/review", (req, res) => {
  ...
  if (!reviewer || !rating) return res.status(400).json({ error: "reviewer and rating required" });
  if (rating < 1 || rating > 5) return res.status(400).json({ error: "rating must be 1-5" });

  agent.reviews.push({ reviewer, rating, comment: comment || "", timestamp: Date.now() });
  res.json({
    status: "reviewed",
    avgRating: +(agent.reviews.reduce((s, r) => s + r.rating, 0) / agent.reviews.length).toFixed(1),
...
app.get("/marketplace/stats", (_req, res) => {
  ...
  res.json({
    ...
    avgReputation: +(agents.reduce((s, a) => s + a.reputation.score, 0) / agents.length).toFixed(0),
    topTags: [...new Set(agents.flatMap(a => a.tags))].slice(0, 10),
    topCapabilities: [...new Set(agents.flatMap(a => a.capabilities))].slice(0, 10),
  });
});
